...
C
      ...
      do i=1,max2
         ...
         ptijmin_active(i)       =.false.
         ptijmax_active(i)       =.false.
         ...
         delta_ymin_active(i)   =.false.
         delta_ymax_active(i)   =.false.
...
C Set cuts as read from file         
         ...
         elseif(buffer(1:13).eq.'delta_ymin ')then
            read(buffer,*) cutname, cclass, cparticle,cclass2,cparticle2,cvalue
            call fill2(cclass,cparticle,cclass2,cparticle2,cvalue,delta_ymin,delta_ymin_active)
         elseif(buffer(1:13).eq.'delta_ymax ')then
            read(buffer,*) cutname, cclass, cparticle,cclass2,cparticle2,cvalue
            call fill2(cclass,cparticle,cclass2,cparticle2,cvalue,delta_ymax,delta_ymax_active)
...
C
      ...
      real*8 e_t,mom_t,y_t,costh_t,phi_t,delta_eta_t,kTij_t,delta_y_t
      real*8 ptij_t
...
c     ptij min cut
          if (ptijmin_active(i))then
             if (ptij_t(p,s2(1,i),s2(2,i)).lt. ptijmin(i)
     &            .or.eventinfo(s2(1,i)/1000,mod(s2(1,i),1000)).eq.0
     &            .or.eventinfo(s2(2,i)/1000,mod(s2(2,i),1000)).eq.0) then
                passcuts=.false.
                return
...
c     ptij max cut
     ...
     &         .and.eventinfo(s2(1,i)/1000,mod(s2(1,i),1000)).ne.0
     &         .and.eventinfo(s2(2,i)/1000,mod(s2(2,i),1000)).ne.0 )then
             if (ptij_t(p,s2(1,i),s2(2,i)).gt.ptijmax(i) )then 
                passcuts=.false.
                return
             endif
          endif
...
c     delta_y min cut
          if (delta_ymin_active(i))then
             if (delta_y_t(p,s2(1,i),s2(2,i)).lt. delta_ymin(i)
     &            .or.eventinfo(s2(1,i)/1000,mod(s2(1,i),1000)).eq.0
     &            .or.eventinfo(s2(2,i)/1000,mod(s2(2,i),1000)).eq.0) then
                passcuts=.false.
                return
...
c     delta_y min cut
          if (delta_ymax_active(i)
     &         .and.eventinfo(s2(1,i)/1000,mod(s2(1,i),1000)).ne.0
     &         .and.eventinfo(s2(2,i)/1000,mod(s2(2,i),1000)).ne.0 )then
             if (delta_y_t(p,s2(1,i),s2(2,i)).gt.delta_ymax(i) )then 
                passcuts=.false.
                return
             endif
          endif
...
c miss Et max cut
     ...
     &        (s4(4,i)/1000.eq.c4.and.mod(s4(4,i),1000).le.p4)     )then
      ...
      do i=1, max2
         if (ptijmin_active(i))then
            call write_cutsnow('ptijmin      ',ptijmin(i),2,i)
         endif
      enddo
      do i=1, max2
         if (ptijmax_active(i))then
            call write_cutsnow('ptijmax      ',ptijmax(i),2,i)
         endif
      enddo
      do i=1, max2
         if (cosijmin_active(i))then
      ...
      enddo
       do i=1, max2
         if (delta_ymin_active(i))then
            call write_cutsnow('delta_ymin  ',delta_ymin(i),2,i)
         endif
      enddo
      do i=1, max2
         if (delta_ymax_active(i))then
            call write_cutsnow('delta_ymax  ',delta_ymax(i),2,i)
         endif
      enddo
      do i=1, max2
