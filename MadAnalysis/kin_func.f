...
c************************************************************************
c     pT of two particle system (added by me)
c************************************************************************
      IMPLICIT none
      double precision p1(0:3),p2(0:3)
...
C****************************************************************************
C     User function for plotting and cuts ==SET TO MASS OF PARTICLE
C****************************************************************************
      implicit none
      double precision p1(0:3)

      X3=((p1(0))**2+(p1(1))**2+(p1(2))**2+(p1(3))**2)
...
C****************************************************************************
C     User function for plotting and cuts == CHANGED TO 3 BODY INVARIANT MASS
C****************************************************************************
      implicit none
      ...
      XYZ1=dsqrt(SumDot3(P1,P2,P3,1d0))

      end

      double precision function XYZ2(p1,p2,p3)
C****************************************************************************
C     User function for plotting and cuts == CHANGED TO 3 BODY PT
C****************************************************************************
      implicit none
      double precision p1(0:3),p2(0:3),p3(0:3)
      double precision p_reco(0:3)
      double precision pt_reco,pt3
      integer i
      
      do i = 0,3
         p_reco(i)=p1(i)+p2(i)
      enddo
      pt_reco = dsqrt((p_reco(1))**2+(p_reco(2))**2)
      pt3 = dsqrt((p3(1))**2+(p3(2))**2)
      XYZ2=pt_reco-pt3
...
C****************************************************************************
C     User function for plotting and cuts == CHANGED TO 3 BODY RAPIDITY
C****************************************************************************
      implicit none
      double precision p1(0:3),p2(0:3),p3(0:3)
      double precision e_reco, p_reco(0:3),y_reco,y3
      integer i
      ...
      enddo
      e_reco = p1(0)+p2(0) 
      y_reco= 0.5d0*dlog((e_reco+p_reco(3))/(e_reco-p_reco(3)))
      y3=0.5d0*dlog((p3(0)+p3(3))/(p3(0)-p3(3)))
      XYZ3=y_reco-y3
...
C****************************************************************************
      implicit none
      double precision p1(0:3),p2(0:3),p3(0:3),p4(0:3),temp(0:3)
      integer mu
      do mu=0,3
         temp(mu)=p1(mu)+p2(mu)+p3(mu)+p4(mu)
      enddo
         
      
      XYZA1=(temp(0)**2-(temp(1)**2+temp(2)**2+temp(3)**2))
      if (XYZA1.ge.0d0) then
         XYZA1=dsqrt(XYZA1)
      else
         XYZA1=0d0
      endif
...
c************************************************************************
      ...
      double precision function SumDot3(P1,P2,P3,dsign)
c********************************************************************
C    Invariant mass of 3 particle system (e.g. lvb , reconstruct W=lv first)
c    added manually by me 
c*******************************************************************
      IMPLICIT NONE
      double precision p1(0:3),p2(0:3),p3(0:3),p_rec(0:3),dsign
      integer i
      double precision ptot(0:3)
      double precision dot
      external dot
      ...
      do i=0,3
         p_rec(i)=p1(i)+dsign*p2(i)
         ptot(i)=p_rec(i)+dsign*p3(i)
      enddo
...
c  four momenta
 ...
 57         continue
            ...
            elseif (plotname(1:4).eq.'mij ')then
               ...
               mijrange(2)=minvalue
               mijrange(3)=maxvalue
            elseif (plotname(1:5).eq.'ptij ')then
               ptijrange(1)=binsize
               ptijrange(2)=minvalue
               ptijrange(3)=maxvalue
            ...
            elseif (plotname(1:10).eq. 'delta_y ')then
               delta_yrange(1)=binsize
               delta_yrange(2)=minvalue
               delta_yrange(3)=maxvalue
...
C
      ...
      double precision function ptij_t(p,ss1,ss2)
      implicit none
      include "info.inc"
      double precision p(0:4,maxparticles),ptij
      integer a1,a2,ss1,ss2
      a1=eventinfo(ss1/1000,mod(ss1,1000))
      a2=eventinfo(ss2/1000,mod(ss2,1000))
      ptij_t=ptij(p(0,a1),p(0,a2))
      return
      end
      ...
      double precision function delta_y_t(p,ss1,ss2)
      implicit none
      include "info.inc"
      double precision p(0:4,maxparticles),delta_y
      integer a1,a2,ss1,ss2
      a1=eventinfo(ss1/1000,mod(ss1,1000))
      a2=eventinfo(ss2/1000,mod(ss2,1000))
      delta_y_t=delta_y(p(0,a1),p(0,a2))
      return
      end
